...
export const getMessLocation = async (messId) => {
  try {
    const messDoc = await getDoc(doc(db, 'messes', messId));
    if (messDoc.exists()) {
      const messData = messDoc.data();
      return messData.location || { lat: 28.6139, lng: 77.2090, address: 'Default Location' };
    }
    return { lat: 28.6139, lng: 77.2090, address: 'Default Location' };
  } catch (error) {
    console.error('Error getting mess location:', error);
    return { lat: 28.6139, lng: 77.2090, address: 'Default Location' };
  }
};